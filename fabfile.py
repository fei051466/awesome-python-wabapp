...
_REMOTE_TMP_DIR = '/tmp/%s' % _TAR_FILE
...
def deploy():
	newdir = 'www-%s' % _now()
	run('rm -f %s' % _REMOTE_TMP_DIR)
	put('dist/%s' % _TAR_FILE, _REMOTE_TMP_DIR)
	with cd(_REMOTE_BASE_DIR):
		sudo('mkdir %s' % newdir)
	with cd('%s/%s' % (_REMOTE_BASE_DIR, newdir)):
		sudo('tar -xzvf %s' % _REMOTE_TMP_DIR)
	with cd(_REMOTE_BASE_DIR):
		sudo('rm -f www')
...
def restore2local():
	...
	p = raw_input('Input mysqll root password: ')
	sqls = [
	...
	for sql in sqls:
		local(r'mysql -u root -p%s -e "%"' % (p, sql))
	with lcd(backup_dir):
