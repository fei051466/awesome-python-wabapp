...
import threading, datetime, re, urllib, os, cgi, mimetypes, functools, logging, types, sys, time, urllib, traceback
...
class Request(object):
	...
	def document_root(self):
		'''
		Get raw docuement_root as str. Return '' if no docuement_root.

		>>> r = Request({'DOCUMENT_ROOT' : '/src/path/to/doc'})
		>>> r.document_root
		'/src/path/to/doc'
		'''
	...
	def environ(self):
		...
		>>> r = Request({'REQUEST_METHOD' : 'GET', 'wsgi.url_schema' : 'http'})
		>>> r.environ.get('REQUEST_METHOD')
		'GET'
		>>> r.environ.get('wsgi.url_schema')
		'http'
	...
	def headers(self):
		'''
		Get all HTTP headers with key as str and value as unicode. The header names are 'XXX-XXX' upercase.
	...
	def header(self, header, default=None):
		'''
		Get header from request as inicode, return None if not exist, or default if specified.
...
class Response(object):
	...
	def content_length(self, value):
		'''
		Set content length, the value can ve int or str.
...
class Jinja2TemplateEngine(TemplateEngine):
	'''
	Render using jiaja2 template engine.
...
def _default_error_handler(e, start_response, is_debug):
	if isinstance(e, HttpError):
		...
		return ('<html><body><h1>%s</h1></html>' % e.status)
	logging.exception('exception')
	start_response('500 Internal Server Error', [('Content-Type', 'text/xml'), _HEADER_X_POWERED_BY])
	if is_debug:
		return _debug()
