...
def user_interceptor(next):
	logging.info('try to bind user form session cookie...')
	user = None
...
def manage_blogs_edit(blog_id):
	...
	if blog is None:
		raise notfound()
	return dict(id=blog.id, name=blog.name, summary=blog.summary, content=blog.content, action='/api/blog/%s' % blog_id, redirect='/manage/blogs', user=ctx.request.user)
...
def api_delete_comment(conmment_id):
	check_admin()
